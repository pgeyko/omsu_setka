...
|----------|-------------|----------|
| **Нет rate-limit middleware** | 🔴 Высокая | В `SPEC.md` описан rate-limit 100 req/min, в `TASK.md` отмечен как `[x]`, но в `middleware.go` — **только логгер и admin-auth**. Нет фактической защиты от DDoS |
