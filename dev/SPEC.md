...
GET /api/v1/changes/:type/:id           → История изменений расписания
```

### 4.3 Уведомления

```
POST /api/v1/subscribe                  → Подписка на уведомления (FCM Token)
POST /api/v1/unsubscribe                → Отписка от уведомлений
```
